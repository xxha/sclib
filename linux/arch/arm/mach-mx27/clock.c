...
//update the mpll .
	...
	mfi = 5;
	mfd = 495;
	reg = (pdf<<CCM_MPCTL0_PD_OFFSET) | (mfn<<CCM_MPCTL0_MFN_OFFSET) |
	(mfi<<CCM_MPCTL0_MFI_OFFSET) | (mfd<<CCM_MPCTL0_MFD_OFFSET);

	__raw_writel(0x01EF15D5, CCM_MPCTL0);
...
//update the spll
	...
	mfi = 8;
	mfd = 117;
	reg=(pdf<<CCM_SPCTL0_PD_OFFSET) | (mfn<<CCM_SPCTL0_MFN_OFFSET)|
	(mfi<<CCM_SPCTL0_MFI_OFFSET) | (mfd<<CCM_SPCTL0_MFD_OFFSET);

	__raw_writel(0x0475206F, CCM_SPCTL0); 
	reg = __raw_readl(CCM_CSCR);
	reg |= CCM_CSCR_SPLLRES;
	reg |= CCM_CSCR_MPLLRES;
	__raw_writel(reg, CCM_CSCR);

	while ((__raw_readl(CCM_MPCTL1) & CCM_MPCTL1_LF) == 0) ;
	while ((__raw_readl(CCM_SPCTL1) & CCM_SPCTL1_LF) == 0) ;
#endif
