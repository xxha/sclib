...
#define	SPI_FIFOSIZE		16	/* Bust size in bytes */
/*
...
static struct mtd_partition s25fl128s_partitions[] = {
	{
	.name = "bootloader",
	.offset = 0,
	...
	{
	.name = "config",
	.offset = 0x00F00000, /* 1MB */
	.size = 0x00100000,
	},
};

static struct flash_platform_data s25fl128s_spi_flash_data = {
	.name = "s25fl128s",
	.parts = s25fl128s_partitions,
	.nr_parts = ARRAY_SIZE(s25fl128s_partitions),
	.type = "s25fl128s",
};
...
static int __devinit m25p_probe(struct spi_device *spi)
{
	...
	flash->mtd.numeraseregions = 1;
	flash->mtd.eraseregions = kmalloc( sizeof(struct mtd_erase_region_info) *
					     flash->mtd.numeraseregions, GFP_KERNEL);
	if (!flash->mtd.eraseregions) {
	...
	flash->mtd.eraseregions[0].erasesize = 0x10000;
	flash->mtd.eraseregions[0].numblocks = 256;
	flash->mtd.eraseregions[0].offset = 0;
