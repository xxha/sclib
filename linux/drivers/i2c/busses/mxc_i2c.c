...
static volatile int slave_addr=0;
/*!
...
static int mxc_i2c_wait_for_tc(mxc_i2c_device * dev, int trans_flag)
{
	...
	if (!tx_success) {
		...
		printk(KERN_EMERG "ACK not received \n");
		return -1;
	}
#endif
	if (!dev->tx_success) {
		/* An ACK was not received for transmitted byte */
		printk(KERN_DEBUG "ACK not received \n");
		//ignore ack for slm board.
		if(slave_addr==0x58){
			if(++bad  >= DEAD_COUNT){
				bad = 0;
				printk("There is a real ACK error!\n");
				return -1;
			}else{
				bad = 0;
				return 0;
			      ...
			      int last)
{
	...
	for(retry=0; retry<10; retry++){	
		...
		for (i = 0; i < len; i++) {
			...
			writew(*buf++, dev->membase + MXC_I2DR);
			//writeb(*buf++, dev->membase + MXC_I2DR);
			if (mxc_i2c_wait_for_tc(dev, msg->flags)) {
				mxc_i2c_stop(dev);
				//printk(KERN_ERR" error occur.%d\n",i);
				break;
				//return -1;
			}
			//msleep(50);
...
static irqreturn_t mxc_i2c_handler(int irq, void *dev_id)
{
	...
	//printk(KERN_EMERG"mxc_i2c_handler ,base:%08x\n",dev->membase);
	sr = readw(dev->membase + MXC_I2SR);
	cr = readw(dev->membase + MXC_I2CR);
//	printk(KERN_EMERG"mxc_i2c_handler ,sr:%08x base cr: %08x\n",sr,cr);
	/*
...
//	mxc_i2c->clkdiv = 0x15;
	...
	printk("i2c speed is %d/%d = %d bps, reg val = 0x%02X\n",
		clk_freq, i2c_clk_table[i].div,
		clk_freq / i2c_clk_table[i].div, mxc_i2c->clkdiv);
