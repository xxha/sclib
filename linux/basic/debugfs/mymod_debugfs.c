...
struct dentry *mydev_maindir = NULL;
struct mydev_debugfs_struct *mydev_debugfs = NULL;
#endif
...
mymod_init (void)
{
	...
	#if defined(CONFIG_DEBUG_FS)
	mydev_debugfs = kcalloc(ndev, sizeof *mydev_debugfs, GFP_KERNEL);

	if (mydev_debugfs && (mydev_maindir = debugfs_create_dir("mydev", NULL))) {
		for (i = 0; i < ndev; i++) {
			if (mydev_ring[i] != NULL) {
				char name[16];
				struct dentry *d;

				sprintf(name, "mydev%d", i);

				if ((d = debugfs_create_dir(name, mydev_maindir))) {
					mydev_debugfs[i].main_dentry = d;
					mydev_debugfs[i].wr_pointer_dentry = debugfs_create_u32("wr_pointer", 0444, d, (u32*)&mydev_ring[i]->wr_pointer);
					mydev_debugfs[i].rd_pointer_dentry = debugfs_create_u32("rd_pointer", 0444, d, (u32*)&mydev_ring[i]->rd_pointer);
					mydev_debugfs[i].data_size_dentry = debugfs_create_u32("data_size", 0444, d, (u32*)&mydev_ring[i]->data_size);
					mydev_debugfs[i].size_dentry = debugfs_create_u32("size", 0444, d, (u32*)&mydev_ring[i]->size);
					mydev_debugfs[i].blob.data = mydev_ring[i]->buffer;
					mydev_debugfs[i].blob.size = mydev_ring[i]->size;
					mydev_debugfs[i].buffer_dentry = debugfs_create_blob("buffer", 0444, d, &mydev_debugfs[i].blob);
				} else
					printk("debugfs_create_dir(%s) returned error %ld\n", name, PTR_ERR(d));
			}
		}
	} else
		printk("debugfs_create_dir() returned error %ld\n", PTR_ERR(mydev_maindir));
	#endif
...
mymod_exit(void) 
{
	...
	#if defined(CONFIG_DEBUG_FS)
	if (mydev_debugfs) {
		if (!IS_ERR_OR_NULL(mydev_maindir)) {
			for (i = 0; i < ndev; i++) {
				if (!IS_ERR_OR_NULL(mydev_debugfs[i].main_dentry)) {
					if (!IS_ERR_OR_NULL(mydev_debugfs[i].wr_pointer_dentry))
						debugfs_remove(mydev_debugfs[i].wr_pointer_dentry);
					if (!IS_ERR_OR_NULL(mydev_debugfs[i].rd_pointer_dentry))
						debugfs_remove(mydev_debugfs[i].rd_pointer_dentry);
					if (!IS_ERR_OR_NULL(mydev_debugfs[i].data_size_dentry))
						debugfs_remove(mydev_debugfs[i].data_size_dentry);
					if (!IS_ERR_OR_NULL(mydev_debugfs[i].size_dentry))
						debugfs_remove(mydev_debugfs[i].size_dentry);
					if (!IS_ERR_OR_NULL(mydev_debugfs[i].buffer_dentry))
						debugfs_remove(mydev_debugfs[i].buffer_dentry);
					debugfs_remove(mydev_debugfs[i].main_dentry);
				}
			}

			debugfs_remove(mydev_maindir);
		}
