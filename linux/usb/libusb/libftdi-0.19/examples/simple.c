/*
	UX400 GPS module simple access.
...
static void ux400_gps_sys_exit()
{
	ftdi_usb_close(&ux400_gps_ftdic);
	ftdi_deinit(&ux400_gps_ftdic);
}
...
static int ux400_gps_write_data(unsigned char * buf, int size)
{
	int ret = 0;

	ret = ftdi_write_data(&ux400_gps_ftdic, buf, size);
