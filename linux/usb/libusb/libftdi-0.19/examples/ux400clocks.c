...
int tcxo_1pps_switch(unsigned int on)
{

	unsigned char data, temp;
	int ret;

	if((ret = Read_bus(&ux400_ftdic, 0x00, REG5_OFFSET, &data, 1)) < 0)
	{
		printf("Read REG5 failed!\n");
		return -1;
	}
	...
	if((ret = Write_bus(&ux400_ftdic, 0x00, REG5_OFFSET, &data, 1)) < 0)
	{
...
int gps_atomic_switch(unsigned int on)
{
...
int main(int argc, char *argv[] )
{
	...
	if(argc != 3){
		printf("usage: ux400clocks 1/0 1/0\n");
		printf("First 1 means TCXO to modules, 0 means 1PPS to modules\n");
		printf("Second 1 means GPS 1PPS to modules, 0 means Atmoic 1PPS to modules\n");
		exit(0);
	}
	...
	if(atoi(argv[1]) == 0){
		tcxo_1pps_switch(0);
		printf("1PPS to modules\n");
	}else{
		tcxo_1pps_switch(1);
		printf("TCXO to modules\n");
	}

	if(atoi(argv[2]) == 0){
		gps_atomic_switch(0);
		printf("Atomic 1PPS to modules\n");
	}else{
		gps_atomic_switch(1);
		printf("GPS 1PPS to modules\n");
	}
