...
int main(int argc, char *argv[] )
{
	...
	if ((ret = ux400_aclock_read_data(buf, sizeof(buf))) >= 0) {
		if(ret > 0){
	...
	printf("Get TOD\n");
	if ((ret = ux400_aclock_write_data("T\n", 1)) < 0) {
		printf("Write Atomic clock failed!\n");
		exit(1);
	}
	...
	printf("Mode checking\n");
	if ((ret = ux400_aclock_write_data("!M?\n", 4)) < 0) {
		printf("Write Atomic clock failed!\n");
		exit(1);
	}
	...
	printf("Disciplining time checking\n");
	if ((ret = ux400_aclock_write_data("!D?\n", 4)) < 0) {
		printf("Write Atomic clock failed!\n");
		exit(1);
	}
	...
	printf("Enable discipline\n");
	if ((ret = ux400_aclock_write_data("!MD\n", 4)) < 0) {
		printf("Write Atomic clock failed!\n");
		exit(1);
	}
...
#if 1
	printf("Enable Auto sync\n");
	if ((ret = ux400_aclock_write_data("!MS\n", 4)) < 0) {
		printf("Write Atomic clock failed!\n");
		exit(1);
	}
...
#if 0
	printf("Start 1PPS sync!\n");

	if ((ret = ux400_aclock_write_data("S", 1)) < 0) {
		printf("Write Atomic clock failed!\n");
		exit(1);
	}
