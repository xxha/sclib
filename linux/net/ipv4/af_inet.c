...
/*tcp timestamp offset info */
typedef struct{
        unsigned int snd_t;
        unsigned int rcv_t;
}HTTP_TIME_INFO;
HTTP_TIME_INFO  *http_time_info;
...
int inet_ioctl(struct socket *sock, unsigned int cmd, unsigned long arg)
{
	...
	switch (cmd) {
		...
		case SIOCSETSEQACK:
			...
			break;
		case SIOCSETFTIME:
			http_time_info = (struct HTTP_TIME_INFO *)arg;
#ifdef VEEX_DEBUG
			printk("changing timestamp\n");
			printk("tsoffset = %08x\n", http_time_info->snd_t);
#endif
			tsoffset = http_time_info->snd_t;
			break;
		default:
			if (sk->sk_prot->ioctl)
