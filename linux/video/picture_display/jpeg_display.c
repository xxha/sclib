...
unsigned int RGB888to32bpp(unsigned char red, unsigned char green, unsigned char blue)
{
	unsigned int B = blue & 0xFF;
	unsigned int G = (green << 8) & 0xFF00;
	unsigned int R = (red << 16) & 0xFF0000;
...
int fb_pixel32(void *fbmem, int width, int height, int x, int y, unsigned int color)
{
	if ((x > width) || (y > height))
		return (-1);
...
int main(int argc, char **argv)
{
	...
	write(f, "\033[9;0]", 8);   //disable console screensave
	write(f, "\033[?25l", 8);   //disable console cursor
	...
	printf("frame buffer:\n	width x height = %dx%d,\n	fb_bpp = %d,\n	fb_size = %d,\n	fb_line_length = %d\n",
		fbdev.fb_width, fbdev.fb_height, fbdev.fb_bpp, fbdev.fb_size, fbdev.fb_line_len);
	...
	while (cinfo.output_scanline < cinfo.output_height)
	{
		...
		if (fbdev.fb_bpp == 16) {
			...
			for (x = 0; x < cinfo.output_width; x++) {
				color32 = RGB888to32bpp(buffer[x * 3],
								buffer[x * 3 + 1], buffer[x * 3 + 2]);
				fb_pixel32(fbdev.fb_mem, fbdev.fb_width, fbdev.fb_height, x, y, color32);
			}
		}
		y++;
	}
